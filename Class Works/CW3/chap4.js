...
function analyze(min = 0) {
  return [...EVENTS]
    ...
    .map(x => x.evt + ": " + x.cor.toFixed(4) + "\t\t\t"
      + `${countEvent(x.evt)}` + "\t\t\t(" + `"${x.evt}"`
      + " occurs in " + `${countEvent(x.evt)}` + " entries)")
}

function countEvent(eventType) {
  let counter = 0;
  JOURNAL.map(x => x.events)
         .filter(element => element.includes(eventType) ? counter += 1 : counter)
  return counter;
}
...
journalEvents()  //determine the set of eventsents